...
import NeitherAbleToUpdateNorToCreateSurveyError
  from '@libs/survey/errors/neither-able-to-update-nor-to-create-survey-error';
...
describe('SurveysController', () => {
  ...
  describe('find', () => {
    ...
    it('should return the commited answer for a survey from user document', async () => {
      ...
      const result = await controller.getCommittedSurveyAnswers({ surveyId: id_FirstMockSurvey, participant: first_username }, 'testUser');
  ...
  describe('updateOrCreateSurvey', () => {
    ...
    it('should throw an error if the survey creation or update fails', async () => {
      ...
      const createDto = { ...secondMockSurvey };
      await expect(controller.updateOrCreateSurvey(createDto, 'testUser')).rejects.toThrow(NeitherAbleToUpdateNorToCreateSurveyError);
    });
  });
  ...
  describe('deleteSurvey', () => {
    ...
    it('should throw an error if the survey removal fails', async () => {
      ...
      await expect(controller.deleteSurvey({ surveyIds: [id_SecondMockSurvey] })).rejects.toThrow(NotAbleToDeleteSurveyError);
    });
  ...
  describe('answerSurvey', () => {
    it('should add an answer to a survey', async () => {
      ...
      const result = await controller.answerSurvey({ surveyId: id_FirstMockSurvey, answer: privateAnswer_FirstMockSurvey }, 'testUser');
      expect(result).toBe(true);
    ...
    it('should throw an error if adding an answer fails', async () => {
      ...
      await expect(controller.answerSurvey({ surveyId: id_FirstMockSurvey, answer: privateAnswer_FirstMockSurvey }, 'testUser')).rejects.toThrow('Survey Error (Adding Answer)');
    });
  });
});
