...
const name = '003-add-schools-field';

const surveyTemplatesMigration003AddSchoolsField: Migration<SurveysTemplateDocument> = {
  name,
  version: 3,
  execute: async (model: Model<SurveysTemplateDocument>) => {
    ...
    const bulkOperations: AnyBulkWriteOperation<SurveysTemplateDocument>[] = unprocessedDocuments.map((document) => ({
      updateOne: {
        // eslint-disable-next-line no-underscore-dangle
        filter: { _id: document._id },
        update: { $set: { schools: [], schemaVersion: newSchemaVersion } },
        upsert: true,
      },
    }));
...
export default surveyTemplatesMigration003AddSchoolsField;
