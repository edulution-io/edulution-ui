...
const migration010: Migration<AppConfig> = {
  ...
  version: 10,
  execute: async (model) => {
    const previousSchemaVersion = 10;
    const newSchemaVersion = 11;
    ...
    const enabledResults = await model.updateMany(
      { schemaVersion: previousSchemaVersion, name: { $in: APPS_WITH_PUSH_NOTIFICATIONS } },
      {
        $set: {
    ...
    const disabledResults = await model.updateMany(
    ...
    Logger.log(
      `Migration completed: ${enabledResults.modifiedCount} apps with push notifications enabled, ${disabledResults.modifiedCount} apps with push notifications disabled`,
    );
  },
};
