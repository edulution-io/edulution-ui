...
const MobileSidebar: React.FC<SidebarProps> = ({ sidebarItems }) => {
  ...
  return (
    <>
      <div
        className="fixed bottom-0 left-0 z-[400] rounded-tr-md bg-secondary px-2 py-1 text-xs text-foreground shadow-xl shadow-slate-400 transition-transform duration-300 ease-in-out dark:bg-black dark:shadow-black"
      ...
      >
        <div
          ref={sidebarRef}
          className={cn(
            'bg-glass fixed right-0 h-full min-w-[260px] border-l-[1px] border-muted  text-foreground shadow-xl shadow-slate-400 backdrop-blur-md',
            sidebarClassName,
          )}
        >
          <div className="h-1" />
