...
import { Collapsible, CollapsibleContent, CollapsibleTrigger } from '@/components/ai-elements/collapsible';
...
export const QueueItemAction = ({ className, ...props }: QueueItemActionProps) => (
  <Button
    className={cn(
      'hover:bg-muted-foreground/10 size-auto rounded p-1 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:text-foreground',
      className,
    )}
    size="icon"
    type="button"
...
export const QueueSectionTrigger = ({ children, className, ...props }: QueueSectionTriggerProps) => (
  <CollapsibleTrigger asChild>
    <button
      className={cn(
        'bg-muted/40 group flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm font-medium text-muted-foreground transition-colors hover:bg-muted',
        className,
      )}
      type="button"
      {...props}
