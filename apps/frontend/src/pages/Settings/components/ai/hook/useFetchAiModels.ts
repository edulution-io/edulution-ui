...
import { AI_CONFIGS_MODELS_ENDPOINT, AI_ENDPOINT } from '@libs/ai/constants/aiEndpoints';
...
const useFetchAiModels = () => {
  ...
  const fetchModels = async (url: string, apiKey: string, apiStandard: string) => {
    ...
    setError(null);
    try {
      const response = await eduApi.post<FetchModelsResult>(`${AI_ENDPOINT}/${AI_CONFIGS_MODELS_ENDPOINT}`, {
        url,
        apiKey,
        apiStandard,
      });
