...
import { AI_CONFIGS_ENDPOINT, AI_ENDPOINT } from '@libs/ai/constants/aiEndpoints';
...
const useAiConfigTableStore = create<AiConfigTableStore>((set, get) => ({
  ...
  fetchTableContent: async () => {
    ...
    try {
      const response = await eduApi.get<AiConfigDto[]>(`${AI_ENDPOINT}/${AI_CONFIGS_ENDPOINT}`);
  ...
  deleteTableEntry: async (_applicationName: string, id: string) => {
    set({ isLoading: true, error: null });
    try {
      await eduApi.delete(`${AI_ENDPOINT}/${AI_CONFIGS_ENDPOINT}/${id}`);
  ...
  addOrUpdateConfig: async (config: AiConfigDto) => {
    ...
    try {
      ...
      if (isUpdate) {
        const response = await eduApi.put<AiConfigDto>(`${AI_ENDPOINT}/${AI_CONFIGS_ENDPOINT}/${config.id}`, config);
        ...
        const response = await eduApi.post<AiConfigDto>(AI_CONFIGS_ENDPOINT, createDto);
        savedConfig = response.data;
        set({
