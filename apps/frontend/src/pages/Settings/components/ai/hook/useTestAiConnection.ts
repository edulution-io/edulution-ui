...
import { AI_CONFIGS_TEST_ENDPOINT, AI_ENDPOINT } from '@libs/ai/constants/aiEndpoints';
...
const useTestAiConnection = () => {
  ...
  const testConnection = async (config: Partial<AiConfigDto>): Promise<AiTestResult> => {
    ...
    try {
      const response = await eduApi.post<AiTestResult>(`${AI_ENDPOINT}/${AI_CONFIGS_TEST_ENDPOINT}`, {
        url,
        apiKey: apiKey || '',
        aiModel,
        apiStandard,
