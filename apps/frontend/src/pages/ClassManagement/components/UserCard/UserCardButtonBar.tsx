...
const UserCardButtonBar = ({ user }: UserCardButtonBarProps) => {
  ...
  const onButtonClick = async (event: React.MouseEvent<HTMLElement>, button: UserCardButton) => {
    ...
    if (button.title === UserCardButtons.Veyon) {
      alert(t('classmanagement.veyonIsStillInDevelopment'));
  ...
  return (
    <>
      {booleanButtons.map((button, index) => (
        <div className="group relative">
          ...
          >
            ...
            <div className="absolute right-0 top-0 hidden h-full items-center justify-center whitespace-nowrap rounded-xl bg-gray-200 px-3 text-foreground group-hover:flex">
              {t(`classmanagement.${button.title}`)}{' '}
              {t(button.value ? 'classmanagement.disable' : 'classmanagement.enable')}
            </div>
          </button>
        </div>
      ))}
