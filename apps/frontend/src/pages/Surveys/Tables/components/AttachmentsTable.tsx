...
const AttachmentsTable = () => {
  ...
  const { setSelectedRows, setSelectedItems, selectedRows, attachments, isLoading } = useSubmittedAnswersDialogStore();
  ...
  const handleRowSelectionChange: OnChangeFn<RowSelectionState> = (updaterOrValue) => {
    ...
    const selectedItemData = Object.keys(newValue)
      ...
      .map((rowId) => attachments.find((attachment) => attachment.filePath === rowId))
  ...
  const initialColumnVisibility = useMemo(
    () => ({
      ...
      [FILE_SHARING_TABLE_COLUMNS.IS_SHARED]: true,
    }),
    [shouldHideColumns],
  );
