...
- [ ] Extract AGENTS.md from edulution-ui into the framework repo
- [ ] Layered .md system: base conventions + project-type-specific overrides (edulution-ui core, custom React apps, styled HTML/CSS/JS pages)
- [ ] AI-agnostic — works with any AI coding tool via AGENTS.md (no Claude-specific files required)
- [ ] Framework repo used as Git submodule at root level (`edulution-ai-framework/`) in all consuming projects
- [ ] edulution-ui itself consumes the framework as a submodule (dog-fooding), removing its inline AGENTS.md entirely
- [ ] Node.js setup script with interactive prompts (inquirer-style) that scaffolds new projects
- [ ] Setup script asks user for: project type (custom app / styled page / core), pre-configurations (Vite+React+TS, Tailwind+theme, auth integration, API client, test framework), and install location
- [ ] Scaffolded React custom apps are ready-to-run with selected pre-configurations
- [ ] Scaffolded styled pages provide Tailwind config + source for building CSS
- [ ] Auth integration scaffolding: SSO on same origin when using edulution API, framework-provided auth flow otherwise
- [ ] API client pre-configuration: axios/eduApi configured to talk to edulution backend
- [ ] Full swagger-spec.json included in framework (synced from edulution-ui via CI)
- [ ] Tailwind config synced from edulution-ui via CI (plus any general-purpose CSS files identified)
- [ ] GitHub Actions pipeline in edulution-ui that syncs swagger spec, tailwind config, and CSS artifacts to the framework repo on merge to dev/main
- [ ] Support ui-kit as both npm package dependency and local checkout
...
- [ ] Linux setup script support (Windows deferred)
...
| -------------------------------- | ------------------------------------------------------------------------------------------ | --------- |
| Git submodule over npm package   | Submodule reference in public repos reveals nothing without access; no publish step needed | — Pending |
| AI-agnostic via AGENTS.md        | Other tools (Cursor, Copilot) can read AGENTS.md directly; CLAUDE.md is just a pointer     | — Pending |
| Node.js setup script             | Cross-platform potential for future Windows support; rich interactive prompts              | — Pending |
| CI sync over manual/hooks        | Tailwind config is stable, swagger changes often; CI on merge is reliable and automated    | — Pending |
| Full swagger spec (not filtered) | AI can find relevant endpoints; filtering adds maintenance burden                          | — Pending |
